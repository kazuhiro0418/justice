...
class Prefecture
  ...
  def show_area_population
    PREF_LIST.each do |i|
      i[:population].delete!(',').to_i
    end
    arr =[]
    sorted_list = PREF_LIST.sort_by { |i| i[:population] }
    sorted_list.each_with_index do |pref, rank|
      arr.push("#{rank + 1}位：#{pref[:name]}")
    end
  arr
  end
end
...
puts prefecture.show_area_population
